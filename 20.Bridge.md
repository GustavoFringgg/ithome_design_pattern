# Day 20｜Bridge 橋接模式 — 什麼是橋？用「組合取代繼承」解開多維度耦合死結
...
4. ConcreteImplementor（具體實作化角色）：把 Implementor 定義的操作做出來的類別

我們來聊聊: 為什麼會有 Bridge Design Pattern?

> 當一個物件同時具備兩個（或以上）獨立變化的維度時，如果只使用物件導向常見的「繼承（Inheritance）」來擴充功能，系統會迅速陷入「類別爆炸（Class Explosion）」的災難
...
所有車型（如 Car、Motorcycle）都能直接搭配使用

反過來，車行要推出「貨車」這個新車型，只要新增一個 `Truck : Vehicle`，也不用去動任何動力來源的程式碼
...
| **ConcreteImplementor** | `FuelEngine`、`ElectricMotor` | 「動力來源」階層底下的具體實作，把 `Power()` 做出來                                                     |

希望以上的舉例可以讓初識 Bridge 的人能有點理解

Bridge 解決的核心問題是：**多用組合，少用繼承**

一個東西身上，如果有兩個（或以上）會各自獨立變化的維度

用單一繼承階層把這些維度壓平、寫死成固定組合的子類別，只要任一維度增加新成員，類別數量就會跟著另一個維度的既有值相乘成長
...
小黑後來要求加上「會員積分報表」，阿柴比照辦理，又多開兩個類別，產出 PDF 跟 Excel
...
ConcreteImplementor => 具體實作化角色(實作匯出格式)
...
- 如果這兩個維度實際上沒有各自獨立變化（例如某些組合根本不會出現、兩者其實高度耦合），硬拆成兩個階層只是徒增複雜度，換不到對應的彈性

---

明天來聊聊遇到大量重複資料，共享不再重複建立， **Flyweight** 享元模式
