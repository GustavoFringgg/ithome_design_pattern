...
繼承本身沒有錯，錯的是繼承了卻沒有守住父類別的行為承諾

也不是叫你看到一點點行為差異，就急著拆成十個小介面，那反而是介面隔離原則（下一篇 I）要處理的過猶不及。判斷標準可以是：**子類別的差異，是「用不同方式完成同一件事」，還是「根本在做另一件事」**

`PercentageDiscount` 跟 `FixedAmountDiscount` 都是「給價格、還折扣後價格」，只是算法不同，這是正常的多型

`BuyOneGetOneDiscount` 起初需要的是「不同的輸入」，這不是同一種行為，硬塞進同一個父類別才會出事

## LSP 容易跟哪個原則搞混

**跟 OCP**：OCP 講的是「新需求進來，能不能用新增子類別/實作去滿足，而不用改舊程式碼」；LSP 講的是「新增出來的那個子類別，能不能真的安全替換掉父類別」
...
如果答案是「要先判斷是哪個子類別，才知道怎麼呼叫」，那大概率就是違反 LSP 的訊號
