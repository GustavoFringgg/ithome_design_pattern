...
// 建立 Factory -> 實作相同介面 -> 丟入參數 -> 回傳建立物件
public class DrinkFactory
{
    public IDrink CreateDrink(string type)
    //使用者不需要知道物件的建立細節，只要傳入對應參數就能拿到需要的物件
    {
        if (type == "latte")
        {
            return new Latte();
...
但阿柴知道，小黑遲早會想加新飲料，例如先前討論過的「燕麥拿鐵」，當新增飲料類別時，也會影響到 `DrinkFactory` 的變更
...
{
    // 這就是 Factory Method（工廠方法）：定義抽象建立介面，由子類別實作具體物件
    public abstract IDrink CreateDrink();

    // 核心業務流程：點餐、製作、結帳
    // 基底類別只依賴抽象的 IDrink，不需要知道實際做出來的是拿鐵還是美式
...
// 2. 具體工廠（Concrete Creators）：每種飲料專屬的製作工廠
...
public class Cashier
{
    ...
    {
        IDrink drink = store.OrderDrink();
        // 這裡可以處理其他通用流程，例如印發票、累計營業額等等
    }
}

// ==========================================
// 4. 程式進入點 / 組裝調用端（Entry Point）
// ==========================================
...
Cashier（業務層）：只知道呼叫 store.OrderDrink()，未來就算新增 100 種飲料，Cashier 也永遠不需要修改或增加 if-else
...
第一種，AI 直接生出小黃當年那種一大包 `if-else`，能動，語法正確，但沒考慮到「這串邏輯以後還會不會變」

第二種，AI 反而過度熱心，一開口就是 `IDrinkStore`、`AbstractDrinkStore`，外加一個 `DrinkStoreRegistry`、`DrinkStoreFactory`，三種飲料包出七八個類別，這是典型的**設計過頭**，因為它不清楚柴咖啡實際的需求

這裡能拿來檢查 AI 產出程式碼的問題只有一個：**現在真的有第二種、第三種產品在排隊，還是只是先包好以防萬一？** 如果連小黑都還沒說要加新飲料，AI 卻已經先幫你切好五層抽象、外加一個註冊機制，那多半是它在幫你腦補一個還沒發生的需求

AI 給得出「Factory Method 該長什麼樣子」的語法，甚至能一次生出 `DrinkStore`、`Cashier`、`Program` 這一整組結構，但答不出「阿柴店裡現在到底要不要用」——這個判斷，還是得靠人自己盯著需求看

---

明天繼續繼續加油，飲料種類要再往外擴一圈:熱飲、冷飲各自要配一整組不同的配件,一次生成,這時候就輪到 **Abstract Factory** 上場了
