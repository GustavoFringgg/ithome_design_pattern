...
看到這裡，問題出在哪呢，**咖啡機只有一台，但軟體卻把它當成「每次點餐都可以生出一條新連線」在用**，連線數越疊越多，使用者感覺卡頓，甚至連線失敗

問題不是「物件怎麼生」，而在**只有一台機器，卻被當成可以無限重新連線的資源，每次點餐都重新握手一次**
...
1. **只有一個實體**：不管在程式裡的哪個角落想拿 `MachineDriver`，拿到的永遠是同一份記憶體位置上的物件
...
public sealed class MachineDriver
{
    ...
    {
        Console.WriteLine("--> 正在連線咖啡機硬體...（只會執行這一次）");
        // 實際上這裡會去開 TCP / Serial 連線，跟真正的咖啡機握手
    }
...
counter.TakeOrder("美式"); // 直接用已經連好的線，零延遲、不會撞連線
```
...
第一次點餐時才建立長連線(Lazy，省下一開機就先佔用連線資源的成本)，之後所有訂單全部共用這個已經連好線的實例(Singleton，零重複連線延遲，也不會有兩條連線搶同一個埠)，因為 `MachineDriver.Instance` 從頭到尾都是同一顆物件，不會再有每次點餐都重新握手、甚至撞線失敗的情況

> 為什麼用 `Lazy<T>`，不直接 `private static MachineDriver _instance = new MachineDriver();`？<br>
...
**變成隱藏依賴**：`OrderCounter` 內部直接寫死 `MachineDriver.Instance`，從建構子完全看不出這個類別依賴了咖啡機連線，這正好違反 Day 7 講的依賴反轉——依賴關係應該要顯式(透過建構子或介面)講清楚，而不是藏在方法內部偷偷取用全域變數
...
現在的柴咖啡只有一台實體咖啡機，`MachineDriver.Instance` 全店共用一份是合理的假設，因為現實裡也真的只有這一台機器可以連。但如果哪天真的展成連鎖店(這系列後面會走到那一步)，每家分店都有自己的實體咖啡機——A 店的 `MachineDriver.Instance` 不能拿去連 B 店的機器，兩者是完全不同的硬體、不同的連線位址

到那個時候，`MachineDriver.Instance` 這種寫死成「整個程式只有一個」的全域單例就會變成阻礙，因為它的假設從一開始就是「全世界只有一台咖啡機」，而不是「每家分店各一台」。要撐住多分店，得改成用依賴注入去管理「每個分店一個連線實體」的生命週期(例如注入容器裡設定成 per-store 的 Scoped 生命週期，每個 store id 對應各自的連線)，而不是繼續用 `static` 硬幹

所以動手寫 Singleton 前，先問自己：**這份資源，是「全域只該有一份」，還是「目前湊巧只有一份」？** 前者才適合 Singleton，後者只是現況剛好如此，用了 Singleton 反而會在規模變化時卡住

## AI 產出程式碼時，Singleton 能拿來檢查什麼

如果阿柴把「幫我寫一個管理咖啡機連線的類別」丟給 AI，同樣容易看到兩種極端

保守版本，AI 可能完全不管「共用連線」這個需求，就寫出一個普通類別，讓 `OrderCounter` 自己每次 `new MachineDriver()`，於是又回到阿柴一開始踩到的那個坑——連線越疊越多、設備衝突，因為 AI 沒有被告知「這條連線本來就該全域只建立一次」

過度發揮版本，AI 則常常見到什麼都想包成 Singleton：`OrderCounter`、`Cashier`、`KitchenService` 全部都被包成 `.Instance`，理由只是「反正比較好拿、少寫一次注入」。但這些類別本身沒有「底層只能接一條實體連線」這種業務理由，純粹是圖方便把 Singleton 當成免注入的全域變數在用，結果是整包程式碼互相用 `.Instance` 勾在一起，日後要拆分、要測試、要因應展店切成多份實體，全部都要重新來過

AI 給得出 Singleton 該長什麼樣子的語法，甚至能一次生出 `Lazy<T>`、private 建構子這些細節都不出錯，但它答不出「柴咖啡裡，到底哪些資源是真的全域唯一(像那台只有一條連線可接的咖啡機)，哪些只是暫時看起來只有一份」——這件事得靠人先把「唯一性」的邊界劃清楚，講給 AI 聽，AI 產出的 Singleton 才會用在真正該用的地方，而不是變成到處都是的全域變數
